...
def new_game():
    ...
    tile_numbers = [ i for i in range(DISTINCT_TILES) ] * 2
    random.shuffle(tile_numbers)
    my_tiles=[]
    for i in range(2*DISTINCT_TILES):
        if i % 2 != 0  :
            my_tiles.append(Tile(tile_numbers[i], False, [TILE_WIDTH * (i//2), TILE_HEIGHT ]) )
        else: 
            my_tiles.append(Tile(tile_numbers[i], False, [TILE_WIDTH * (i/2), 2 * TILE_HEIGHT ]) )
